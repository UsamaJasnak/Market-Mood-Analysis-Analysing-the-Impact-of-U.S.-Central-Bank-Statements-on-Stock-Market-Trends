...
class SentimentLSTM(nn.Module):
    def __init__(self,
                 ...
                 output_dim, n_layers, dropout_lstm, dropout_h_c_states,
                 pad_idx, pos_pad_idx,
        ...
        self.merge_hidden_cell = nn.Linear(2*self.hidden_dim, self.hidden_dim)
    ...
    def forward(self, text, pos_tags, text_lengths):
        ...
        cell = cell[-1, :, :]      # [batch size, cell_dim]

        if self.cell_state_info == 'linear':
            hidden_cell = self.merge_hidden_cell(torch.cat([hidden, cell], dim=1))  # [batch size, 2*hidden_dim]
        elif self.cell_state_info == 'add':
            hidden_cell = hidden + cell
        else:
            hidden_cell = hidden  # [batch size, hidden_dim]

        output = self.fc(hidden_cell)  # [batch size, output_dim]
...
def train_model(model, loader, criterion, optimizer, device='cuda'):
    """
...
def test_model(model, loader, criterion, device='cuda'):
    """
